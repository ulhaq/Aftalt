...
if(!empty($q)){
$rs = mysql_query("SELECT * FROM tasks WHERE (name LIKE '%$q%' OR budget LIKE '%$q%' OR deadline LIKE '%$q%' OR location LIKE '%$q%' OR comp LIKE '%$q%' OR date LIKE '%$q%') AND active='1' ORDER BY time DESC LIMIT 0,$limit");
$numrowslimit = mysql_num_rows($rs);
$numrows = mysql_num_rows(mysql_query("SELECT * FROM tasks WHERE (name LIKE '%$q%' OR budget LIKE '%$q%' OR deadline LIKE '%$q%' OR location LIKE '%$q%' OR comp LIKE '%$q%' OR date LIKE '%$q%') AND active='1'"));
if($numrows>0){
$a = $limit;
if ($a > $numrows){
$a = $numrows;
...
echo "
<li class='rsListed' onclick='javascript:location.href=\"tasks/$row[tname]\"' onmouseover='javascript:this.style.cursor=\"pointer\"'>
<div class='rsTop'>
<a href='tasks/$row[tname]' class='rsTitle tool'>";
...
$aRs = mysql_query("SELECT * FROM tasks WHERE active='1' ORDER BY time DESC LIMIT 0,$limit");
$rsNumL= mysql_num_rows($aRs);
$rsNum = mysql_num_rows(mysql_query("SELECT * FROM tasks WHERE active='1' ORDER BY time DESC"));
if($rsNum>0){
$a = $limit;
if ($a > $rsNum){
$a = $rsNum;
...
<li class='rsListed' onclick='javascript:location.href=\"tasks/$row[tname]\"' onmouseover='javascript:this.style.cursor=\"pointer\"'>
<div class='rsTop'>
<a href='tasks/$row[tname]' class='rsTitle tool'>$row[name]<span class='tip t l'>Opgavetitle<span class='tarrow at l'></span></span></a>
<span class='rsDate tool'>";
cpTime($row['time'],$row['date'],"%d-%m-%Y");
echo "<span class='tip t l'>Tid<span class='tarrow at l'></span></span></span>
</div>
